...
try:
    ...
    pass
# 2️⃣ SIGN (pakai wallet private key Agent EOA)
...
def is_team_alone(state):
    """
    Cek apakah hanya agen tim kita yang tersisa di map (tidak ada musuh & guardian)
    """
    for agent in state.get("visibleAgents", []):
        if not is_teammate(agent.get("name")) and not is_guardian(agent):
            return False  # Masih ada musuh
    return True  # Hanya tim kita tersisa
...
def choose_goal(state):
    ...
    # =========================
    # 🔹 CEK TIM SENDIRI SAJA
    # =========================
    if is_team_alone(state):
        # Diam atau kumpul tim → tidak menyerang
        safe_region = get_team_center(state) or current
        return {"target": safe_region, "type": "idle"}
...
def attach_wallet(wallet_address):
    payload = {"walletAddress": wallet_address}
    url = f"{BASE_URL}/accounts/me/wallets"
    ...
    if res and res.status_code == 200:
        data = res.json()
        if data.get("success"):
            print(f"✅ Wallet attached: {wallet_address}")
            return True
        else:
            print(f"⚠ Gagal attach wallet: {data}")
    else:
        print(f"❌ Attach wallet gagal, status_code: {res.status_code if res else 'None'}")
